...
{\color{red}{
In Figure~\ref{fig:drawback} and \ref{fig:benefit},
The performance drops slightly as the matrix size increases 
under $NT \ge 16$.
The reason is that there is a synchronization point
...
it increases quickly as $NT$ increases.
Performance also drops at matrix sizes that are
multiples of 1024 because
GEMM suffers from more memory contention at these sizes,.
As an example, we profiled the GEMM execution
at matrix sizes 3968 and 4096 under $NT_C=4$ and $NC=4$.
The measured bandwidth for packing A (B) are
1.44(0.89) GB/s at size 3968 and 0.81(0.44) GB/s at size 4096.
We can see that there is a big performance gap
in memory bandwidth.
Due to the organization of the DDR memory hardware,
The selected tiling factors $M_c$, $N_c$ and $K_c$
made GEMM more likely to cause memory contention at sizes
...
i.e., the SCP methodology can apply transparently to 
other BLAS-3 routines as well.
}}
